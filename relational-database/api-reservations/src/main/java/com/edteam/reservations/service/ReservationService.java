...
public class ReservationService {
    ...
    public Mono<ReservationDTO> getReservationById(Long id) {
        ...
        return Mono.justOrEmpty(conversionService.convert(result.get(), ReservationDTO.class)).map(dto -> {
            if (dto.getPassengers() == null)
                dto.setPassengers(new ArrayList<>());
            return dto;
        });
    ...
    public Mono<ReservationDTO> save(ReservationDTO reservation) {
        ...
        return Mono.justOrEmpty(conversionService.convert(result, ReservationDTO.class)).map(dto -> {
            if (dto.getPassengers() == null)
                dto.setPassengers(new ArrayList<>());
            return dto;
        });
    ...
    private Flux<ReservationDTO> getDiscountedReservations(SearchReservationCriteriaDTO criteria) {
        ...
        ItineraryDTO itinerary = new ItineraryDTO(1L, 2L,
                List.of(new SegmentDTO("DEF", "DEF", "2024-01-01", "2024-01-01", "XX")),
                new PriceDTO(BigDecimal.valueOf(10L), BigDecimal.valueOf(5L), BigDecimal.valueOf(5L)));

        // Crear "ReservationDTO" con datos específicos para el flujo simulado
        List<ReservationDTO> discountedReservations = List.of(new ReservationDTO(1001L, // id
                1L, // versión
                List.of(passenger1, passenger2), // pasajeros
                itinerary, // itinerario
                LocalDate.now().minusDays(10) // fecha de creación
        ), new ReservationDTO(1002L, // id
                1L, // versión
                null, // pasajeros
                itinerary, // itinerario
